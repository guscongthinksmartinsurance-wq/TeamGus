...
# --- 3. CÔNG CỤ 3: CALL LOG (KHÔI PHỤC VINH DANH) ---
def process_call_log(file):
    ...
    try:
        ...
        # Vinh danh 5 Top Call (Thứ tự 4-2-1-3-5)
        top_5_call = stat.head(5).copy()
        ...
        for i, item in enumerate(d_map):
            idx = item['i']
            if idx < len(top_5_call):
                row = top_5_call.iloc[idx]
                is_top = (idx == 0)
                with cols[i]:
...
# --- 4. CÔNG CỤ 1 & 2: PHÂN TÍCH TEAM G ---
